...
class Tester(object):
    ...
    def test_classifier(self, dataset, save_results=True, filename="test_results.txt", plot_roc=False, roc_filename="roc_curve.png"):
        N = len(dataset)
        P, C, SMILES_list = [], [], []
        for i in range(0, N, batch_test):
            data_batch = list(zip(*dataset[i:i+batch_test]))
            smiles_batch = data_batch[-1]  # Lấy SMILES từ batch
            predicted_scores, correct_labels = self.model.forward_classifier(data_batch, train=False)
            predicted_scores = F.sigmoid(torch.tensor(predicted_scores)).numpy()  # Áp dụng sigmoid để đưa giá trị về khoảng [0,1]
            P.append(predicted_scores)
            C.append(correct_labels)
            SMILES_list.extend(smiles_batch)

        predicted_scores = np.concatenate(P)
        correct_labels = np.concatenate(C)
        ...
        # Vẽ đường cong ROC nếu được yêu cầu
        if plot_roc:
            fpr, tpr, _ = roc_curve(correct_labels, predicted_scores)
            plt.figure(figsize=(8, 6))
            plt.plot(fpr, tpr, color='blue', lw=2, label=f'ROC curve (AUC = {AUC:.2f})')
            plt.plot([0, 1], [0, 1], color='grey', linestyle='--')  # Đường chéo ngẫu nhiên
            plt.xlim([0.0, 1.0])
            plt.ylim([0.0, 1.05])
        ...
        if save_results:
            with open(filename, "w") as f:
                f.write("SMILES\tPredicted Score\tPredicted Label\tActual Label\n")
                for smiles, score, pred, actual in zip(SMILES_list, predicted_scores, predicted_labels, correct_labels):
                    f.write(f"{smiles}\t{score:.4f}\t{pred}\t{actual}\n")
...
if __name__ == "__main__":
    ...
    if task == 'classification':
      result_header = ('Epoch\tTime(sec)\tLoss_train\t'
                    'AUC_dev\tAcc_dev\tPrec_dev\tRecall_dev\tF1_dev\t'
                    'AUC_test\tAcc_test\tPrec_test\tRecall_test\tF1_test')
    if task == 'regression':
    ...
    for epoch in range(iteration):
        ...
        if task == 'classification':
          AUC_dev, acc_dev, prec_dev, rec_dev, f1_dev = tester.test_classifier(dataset_dev, 
                                                                         save_results=True, 
                                                                         filename="../output/dev_results.txt", 
                                                                         plot_roc=False)  
          AUC_test, acc_test, prec_test, rec_test, f1_test = tester.test_classifier(dataset_test)
        if task == 'regression':
        ...
        result = '\t'.join(map(str, [epoch, time, loss_train,
                             AUC_dev, acc_dev, prec_dev, rec_dev, f1_dev, 
                             AUC_test, acc_test, prec_test, rec_test, f1_test]))
